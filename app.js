...
function renderCard(r) {
  ...
  return `<article class="record record-clickable ${needsClassify ? "needsClassify" : ""} ${aiProposal ? "hasAiProposal" : ""}" data-record-id="${escapeHtml(rid)}" tabindex="0" role="button" aria-label="Klasifikovat: ${escapeHtml(r.title || r.predmet)}">
    ...
    <div class="badges">${excluded ? `<span class="badge excluded">Vyřazeno</span>` : ""}${aiProposal ? `<span class="badge aiProposal">Ke kontrole AI</span>` : needsClassify ? `<span class="badge classify">K roztřídění</span>` : ""}${r.typ ? `<span class="badge ${risk ? "risk" : ""}">${escapeHtml(r.typ)}</span>` : ""}${r.kam_patri ? `<span class="badge ${meeting ? "meeting" : ""}">${escapeHtml(r.kam_patri)}</span>` : ""}${r.priorita ? `<span class="badge priority">${escapeHtml(r.priorita)}</span>` : ""}${taskExported}</div>
